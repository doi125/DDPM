...
def Downsample(dim, dim_out=None):
    # プーリングやストライド付き畳み込みを使用しないダウンサンプリング
    # 高さと幅をそれぞれ1/2にして、チャネル数を4倍に
    return nn.Sequential(
...
class ResnetBlock(nn.Module):
    """ResNetブロック"""
    # WeightStandardizedConv2d → GroupNorm → SiLU → WeightStandardizedConv2d → GroupNorm → SiLU → 残差接続 (scale_shift = Noneの時)
    ...
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        block_klass(dim_in, dim_in, time_emb_dim=time_dim), # ResnetBlock(引数groupsを固定)
                        block_klass(dim_in, dim_in, time_emb_dim=time_dim), # ResnetBlock(引数groupsを固定)
                        Residual(PreNorm(dim_in, LinearAttention(dim_in))), # GroupNorm → LinearAttention → 残差接続
                        Downsample(dim_in, dim_out) # 高さと幅をそれぞれ1/2にして、チャネル数を4倍に
                        if not is_last
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        ...
                        block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim),
                        Residual(PreNorm(dim_out, LinearAttention(dim_out))), # GroupNorm → LinearAttention → 残差接続
                        Upsample(dim_out, dim_in) # 最近傍補間でアップサンプリング(高さと幅それぞれ2倍)
                        if not is_last
